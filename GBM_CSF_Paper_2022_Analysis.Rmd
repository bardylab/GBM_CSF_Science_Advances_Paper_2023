...
cell_line_directory <- paste0(datadirectory, "/RData/Individual Cell Line RData/")
...
2.  ***Load the required RData files that have been previously QC***

```{Load RData files that have been previously QC}
##Load the required TME and CSF libraries, these have been previously QC'd
file_names <- as.list(dir(cell_line_directory))
file_names <- lapply(file_names, function(x) paste0(cell_line_directory, x))
...
##CSF_Markers <- read.csv(file = paste(resultsdirectory, "/Differential Expression/GSEA_differentialexpression-all.csv"))
...
```{r Conduct DE for Proliferation and MES-like}
...
## FIGURE 4A, E and F: Ridge plot of MESlike and MESlike 1, 2 Scores.####
...
## FIGURE 4C and D: Proliferation and MES-like scatter plots with ggplot2. ####
int_breaks <- function(x, n = 5) pretty(x, n)[pretty(x, n) %% 1 == 0] ## function to generate only whole-integer breaks for x and y axis ticks
...
## FIGURE 4J and K: Recreate Neftel meta-module plots with ggplot2. ####
library(ggplot2)
library(ggpointdensity)
Bardy_10x$Condition <- factor(Bardy_10x$Condition, levels = c("TME", "CSF")) ##Set ;evels for plotting order. 
int_breaks <- function(x, n = 5) pretty(x, n)[pretty(x, n) %% 1 == 0] ## function to generate only whole-integer breaks for x and y axis ticks
...
## SUPP FIGURE 6E: Violin Plot showing expression of NUPR1 across quadrants. ####
