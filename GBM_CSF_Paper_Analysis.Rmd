...
13.We visualise the expression of potential NUPR1 pathway markers
...
## Subset seurat object for top marker genes (top 20 per cluster)
subset.matrix <- seurat_cluster.averages[["RNA"]][c("NUPR1","RNF2","TP53INP1","TP53","NSA2","PTMA","EP300","MSL1","NACA","SYNCRIP","NONO","FUS","COPS5","PSMD4","MPHOSPH6","TRIB3","STX18","KDM3A","UCP1","KNSTRN","MAPRE1","RAN","RANBP1","SEC11A"),]
dim(subset.matrix)
cor_matrix <- cor(t(subset.matrix), method = "spearman")
## Z-score the gene expression for visualization with heatmap
cal_z_score <- function(x){ ## function for scaling
  (x - mean(x)) / sd(x)
}
subset.matrix <- t(apply(subset.matrix, 1, cal_z_score))

## Create Heatmap annotations
tbl_GBM_700cells <- GBM_700cells@meta.data %>%
  group_by(Condition_PatientID, Quadrant) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n))
tbl_GBM_700cells

anno <- as.data.frame(tbl_GBM_700cells)
anno <- reshape2::dcast(anno, Condition_PatientID ~ Quadrant)
row.names(anno) <- anno$Condition_PatientID
anno$Condition_PatientID <- NULL
head(anno)
anno[is.na(anno)] <- 0
anno <- (anno / rowSums(anno)) * 100
anno <- anno[match(colnames(subset.matrix),rownames(anno)),]
row.names(anno) == colnames(subset.matrix) ## TRUE
column_ha <- HeatmapAnnotation(Quadrant = anno_barplot(anno, gp = gpar(fill = c("goldenrod", ##red
                                                                                "red",
                                                                                "saddlebrown",
                                                                                "royalblue1")))) ##GBM_700cells

tbl_GBM_700cells_2 <- GBM_700cells@meta.data %>%
  group_by(Condition_PatientID, Cell_Cycle) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n))
tbl_GBM_700cells_2

anno_2 <- as.data.frame(tbl_GBM_700cells_2)
anno_2 <- reshape2::dcast(anno_2, Condition_PatientID ~ Cell_Cycle)
row.names(anno_2) <- anno_2$Condition_PatientID
anno_2$Condition_PatientID <- NULL
head(anno_2)
anno_2[is.na(anno_2)] <- 0
anno_2 <- (anno_2 / rowSums(anno_2)) * 100
anno_2 <- anno_2[match(colnames(subset.matrix),rownames(anno_2)),]
row.names(anno_2) == colnames(subset.matrix) ## TRUE
column_ha_2 <- HeatmapAnnotation(Cell_Cycle = anno_barplot(anno_2, gp = gpar(fill = c("blue","red")))) 

column_ha_3 <- HeatmapAnnotation(NUPR1 = NUPR1_avg[,1:20], col = list(NUPR1 = colorRamp2(c(-1.5, 0, 2), c("white", "coral", "red"))), border = T, annotation_height = unit(1, "cm"),
    annotation_width = unit(1, "cm"))

## Create Heatmap
Names_order <- c("SANTB00111_CSF","SANTB00134_CSF","SANTB00159_CSF","SANTB00448_CSF","SANTB00468_CSF", "SANTB00469_CSF","SANTB00497_CSF","BAH1_CSF","HW1_CSF","MN1_CSF","SANTB00111_TME", "SANTB00134_TME","SANTB00159_TME","SANTB00448_TME","SANTB00468_TME", "SANTB00469_TME","SANTB00497_TME","BAH1_TME","HW1_TME","MN1_TME")


hm <- ComplexHeatmap::Heatmap(subset.matrix,
                              border = TRUE,
                              show_row_names = T,
                              show_column_names = T,
                              row_title = "NUPR1 pathway markers",
                              cluster_rows = TRUE,
                              cluster_columns = T,
                              ##column_order = Names_order,
                              ...
                              bottom_annotation = c(column_ha, column_ha_2),
                              heatmap_legend_param = list(
                                title = "Scaled average expression"))
pdf(file = paste(resultsdirectory,"plots/Differential expression/Heatmap/GBM_700cells_NUPR1_pathway_markers_byConditionPatientID.pdf", sep = "/"), width = 12, height = 12)
##pdf(file = paste(resultsdirectory,"plots/Differential expression/Heatmap/GBM_700cells_NUPR1_pathway_markers_avgexpcorrelation_spearman.pdf", sep = "/"), width = 12, height = 12)
draw(hm)
dev.off()
...
## Subset seurat object for NUPR1 pathway markers
NUPR1_pathway_markers <- c("NUPR1","RNF2","TP53INP1","TP53","NSA2","PTMA","EP300","MSL1","NACA","SYNCRIP","NONO","FUS","COPS5","PSMD4","MPHOSPH6","TRIB3","STX18","KDM3A","KNSTRN","MAPRE1","RAN","RANBP1","SEC11A","UCP1") ##from string

##NUPR1_pathway_markers <- c("NUPR1","MGMT","RELB","CDKN1A","SNAP25","BECN1", "RAB31", "GREB1", "CYP1B1", "NEDD9", "GRN", "DNMT1")

CSF_Markers <- read.csv(file = paste0(resultsdirectory,"/Differential Expression/All_CSF_Markers_700cells_percondition_downsample-minpct0.1.csv"))

subset.matrix <- CSF_Markers[rownames(CSF_Markers) %in% NUPR1_pathway_markers, c(1:2)]
row.names(subset.matrix) <- rownames(subset.matrix)
dim(subset.matrix)

color_h <- colorRamp2(c(-0.5,0,1.6), c("blue","white","red"))
hm <- ComplexHeatmap::Heatmap(as.matrix(subset.matrix),
                              border = TRUE,
                              show_row_names = T,
                              show_column_names = F,
                              cluster_rows = TRUE,
                              ...
                              row_dend_width = unit(1, "cm"),
                              cell_fun = function(j, i, x, y, width, height, fill) {
        grid.text(sprintf("%.1f", subset.matrix[i, j]), x, y, gp = gpar(fontsize = 14))},
             heatmap_legend_param = list(
               title = "Average Log2 FC", at = c(-5,-2.5,0,2.5,5)))
pdf(file = paste(resultsdirectory,"plots/Differential expression/Heatmap/NUPR1_pathway_markers_LOG2FC_allCSF.pdf", sep = "/"), width = 4, height = 12)
draw(hm)
dev.off()

## Load DE_markers
set.seed(1)
file_names <- list.files(paste0(resultsdirectory, "/Differential Expression/"), pattern = "_minpct0.1*",full.names = F)
DE_markers <- lapply(file_names, function(x) {
  read.csv(paste0(resultsdirectory, "/Differential Expression/", x), header = T)}) 
names(DE_markers) <- file_names
rm(file_names)

for(i in 1:length(DE_markers)){
DE_markers[[i]] <- DE_markers[[i]][DE_markers[[i]]$X %in% NUPR1_pathway_markers,c(1,3)]
row.names(DE_markers[[i]]) <- DE_markers[[i]]$X
DE_markers[[i]]$X <- NULL
}

##Create a matrix containing the top genes from the differential expression of all cells
Fold_change <- data.frame(matrix(ncol = 10, nrow=24))
col_names <- c("111", "134", "159", "448", "468", "469", "497", "BAH1","HW1", "MN1")
colnames(Fold_change) <- col_names
rownames(Fold_change) <- NUPR1_pathway_markers
rownames(Fold_change) <- c("NUPR1","MGMT","RELB","CDKN1A","SNAP25","BECN1", "RAB31", "GREB1", "CYP1B1", "NEDD9", "GRN", "DNMT1")

##We now subset the differential expression analysis based on the top genes
for(i in 1:ncol(Fold_change)){
  Fold_change[match(rownames(DE_markers[[i]]),rownames(Fold_change)), i] <- cbind(DE_markers[[i]]$avg_log2FC)
}
Fold_change <- Fold_change[-c(6,9,24),]


color_h <- colorRamp2(c(-1,0,2.5), c("blue","white","red"))
h <- Heatmap(as.matrix(Fold_change), show_row_names = TRUE,
             show_column_names = T, 
             cluster_columns = T, 
             column_dend_reorder = T,
             row_dend_reorder = T,
             ...
             rect_gp = gpar(col = "white", lwd = 1),
             cell_fun = function(j, i, x, y, width, height, fill) {
        grid.text(sprintf("%.1f", Fold_change[i, j]), x, y, gp = gpar(fontsize = 14))},
             heatmap_legend_param = list(
               title = "Average Log2 FC", at = c(-1,0,1,2,3)))
pdf(file = paste0(resultsdirectory, "plots/Heatmap/Heatmap_2_Downsampled.pdf"), width = 12, height = 12, onefile = T) ##Run to save as pdf
draw(h)
dev.off()
...
top_genes <- CSF_Markers %>% top_n(n = 50, wt = avg_log2FC)
top_genes <- top_genes$X

## Subset seurat object for top marker genes (top 20 per cluster)
subset.matrix <- seurat_cluster.averages[["RNA"]][top_genes,]
...
subset.matrix <- t(apply(subset.matrix, 1, cal_z_score))
subset.matrix <- subset.matrix[,-grep("TME", colnames(subset.matrix))]
...
color_h <- colorRamp2(c(-1,-0.5,0,0.5,1), c("blue","deepskyblue","white","coral","firebrick"))
hm <- ComplexHeatmap::Heatmap(subset.matrix,
                              border = TRUE,
                              show_row_names = T,
                              show_column_names = T,
                              ...
                              row_names_gp =  gpar(fontsize = 12),
                              column_dend_height = unit(1, "cm"),
                              row_dend_width = unit(1, "cm"),cell_fun = function(j, i, x, y, width, height, fill) {
        grid.text(sprintf("%.1f", subset.matrix[i, j]), x, y, gp = gpar(fontsize = 0))},
             heatmap_legend_param = list(
               title = "Scaled average expression"), at = c(-5,-2.5,0,2.5,5))
pdf(file = paste(resultsdirectory,"plots/Differential expression/Heatmap/NUPR1_top50genes_CSF_scaledavgexpression.pdf", sep = "/"), width = 12, height = 12)
draw(hm)
dev.off()
...
Phenotype <- msigdbr(species = "Homo sapiens", category = "C5", subcategory = "HPO")
Phenotype <- Phenotype %>% 
...
Oncogenic <- Oncogenic %>% 
  dplyr::select(gs_name, gene_symbol,entrez_gene, ensembl_gene)
Oncogenic <-  split(as.character(Oncogenic$gene_symbol), Oncogenic$gs_name)

Ontology <- msigdbr(species = "Homo sapiens", category = "C5", subcategory = "BP")
Ontology <- Ontology %>% 
  dplyr::select(gs_name, gene_symbol, entrez_gene, ensembl_gene)
Ontology <-  split(as.character(Ontology$gene_symbol), Ontology$gs_name)

fgsea_Ont <- fgsea(Ontology, geneList)
fgsea_Ont <- fgsea_Ont[fgsea_Ont$padj <= 0.05 & fgsea_Ont$size >= 10,]
...
go_cluster <- simplifyGO(go_mat, method = "kmeans") ##8 clusters
...
ifelse(enrichGO_results$Cluster_louvain == "4", "growth and development", 
...
enrichDO <- enrichDO(gene = entrez_genes$ENTREZID,
           universe = all_genes_entrez$ENTREZID,
                ont           = "DO",
                pvalueCutoff  = 0.05,
                qvalueCutoff  = 0.01,
                readable      = TRUE)
save(enrichGO, file = paste(resultsdirectory, "/GSEA/DOenrichment_ORA_percluster_GBM_700cells_top20_updownregulated.RData", sep = "")) ## saved 010223
write.csv(enrichGO@result, file = paste(resultsdirectory, "/GSEA/sheets/DOenrichment_ORA_percluster_GBM_700cells_top20_updownregulated.csv", sep = "")) ## saved 010223

library(simplifyEnrichment)
enrichDO_results <- enrichDO@result[enrichDO@result$p.adjust <= 0.05,]
enrichDO_results <- enrichDO_results[!duplicated(enrichDO_results$geneID),] ## remove any duplicated geneIDs
do_id <- enrichDO_results$ID
do_mat <- DO_similarity(do_id, measure = "Wang")


## See how many clusters can be generated 
clt <- cmp_make_clusters(do_mat)
do_cluster <- simplifyEnrichment(do_mat, method = "kmeans", plot = F) ##8 clusters
do_cluster_louvain <- simplifyEnrichment(do_mat, method = "louvain", plot = F) ## 8 clusters

enrichDO_results$Cluster_kmeans <- do_cluster$cluster[match(enrichDO_results$ID, do_cluster$id)]
enrichDO_results$Cluster_kmeans <- do_cluster$cluster[match(enrichDO_results$ID, do_cluster$id)]

enrichDO_results$Cluster_louvain <- do_cluster_louvain$cluster[match(enrichDO_results$ID, do_cluster_louvain$id)]
enrichDO_results$Cluster_louvain <- do_cluster_louvain$cluster[match(enrichDO_results$ID, do_cluster_louvain$id)]

## Create similarity matrix
ha_louvain = HeatmapAnnotation(clusters = do_cluster_louvain$cluster, 
                       col = list(clusters = c("1" = "coral",
  "2" = "#5BE885",
  "3" = "purple",
  "4" = "goldenrod1",
...
ha_kmeans = HeatmapAnnotation(clusters = do_cluster$cluster, 
                       col = list(clusters = c("1" = "coral",
  "2" = "#5BE885",
  "3" = "purple",
  "4" = "goldenrod1",
...
word_cloud_louvain <- rowAnnotation(DO_term = anno_word_cloud(enrichDO_results$Cluster_louvain, enrichDO_results$Description))
word_cloud_kmeans <- rowAnnotation(DO_term = anno_word_cloud(enrichDO_results$Cluster_kmeans, enrichDO_results$Description))

pdf(file = paste(resultsdirectory,"plots/GO and GSEA/DO_similarity_matrix_kmeans_clustering.pdf", sep = "/"), width = 12, height = 12)
hm <- Heatmap(do_mat,
        col = c("white","red","firebrick"),
        row_split = do_cluster$cluster,
        column_split = do_cluster$cluster,
        show_row_names = F,
        show_column_names = F,
        show_row_dend = F,
        show_column_dend = F,
        ...
        cluster_rows = F,
        cluster_columns = F,
        top_annotation = ha_kmeans,
        row_title = "cluster",
        column_title = "cluster", 
        width = unit(15, "cm"),
        height = unit(15, "cm"),
        right_annotation = word_cloud_kmeans,
       row_names_gp =  gpar(fontsize = 12),
       ##column_dend_height = unit(1, "cm"),
       ##row_dend_width = unit(1, "cm"),
       heatmap_legend_param = list(title = "Similarity"))
...
#####
enrichDO_results$new_description <- ifelse(enrichDO_results$Cluster_louvain == "1", "cell migration", ifelse(enrichDO_results$Cluster_louvain == "2", "response to extracellular signals", ifelse(enrichDO_results$Cluster_louvain == "3", "metabolic processes", 
ifelse(enrichDO_results$Cluster_louvain == "4", "growth and development", 
ifelse(enrichDO_results$Cluster_louvain == "5", "regulation of apoptotic processes", 
ifelse(enrichDO_results$Cluster_louvain == "6", "endoplasmic reticulum stress response",
ifelse(enrichDO_results$Cluster_louvain == "7", "negative growth regulation","cell division")))))))

enrichDO <- pairwise_termsim(enrichDO)
heatplot(enrichDO, foldChange = geneList, showCategory = 50)
cnetplot(enrichDO, foldChange=geneList, circular = TRUE, showCategory = 50)
```

```{Assessing expression of NUPR1 pathway markers}
geneList <- CSF_Markers$avg_log2FC
names(geneList) <- rownames(CSF_Markers)

## Investigate whether MGMT, and other NUPR1 targets are up/downregulated in CSF
Idents(GBM_700cells) <- "Condition_PatientID"
seurat_cluster.averages <- AverageExpression(GBM_700cells, return.seurat = FALSE)
seurat_cluster.averages

## Subset seurat object for top marker genes (top 20 per cluster)
subset.matrix <- seurat_cluster.averages[["RNA"]][c("NUPR1","RNF2","TP53INP1","TP53","NSA2","PTMA","EP300","MSL1","NACA","SYNCRIP","NONO","FUS","COPS5","PSMD4","MPHOSPH6","TRIB3","STX18","KDM3A","UCP1","KNSTRN","MAPRE1","RAN","RANBP1","SEC11A","MGMT","RELB","CDKN1A","HMOX1","SNAP25", "BECN1", "RAB31", "GREB1", "CYP1B1", "NEDD9", "GRN", "DNMT1"),]

write.csv(subset.matrix, file = paste0(resultsdirectory, "/GBM_All_sheets/MGMT_and_NUPR1_marker_average_expression_GBM_700cells.csv"))

subset.matrix <- seurat_cluster.averages[["RNA"]][c("NUPR1","MGMT","RELB","CDKN1A","SNAP25","BECN1", "RAB31", "GREB1", "CYP1B1", "NEDD9", "GRN", "DNMT1"),]
...
subset.matrix <- do.call("rbind", subset.matrix_FC)
subset.matrix[,1:12][is.infinite(subset.matrix[,1:12])]
...
hm <- ComplexHeatmap::Heatmap(as.matrix(subset.matrix),
        ...
        grid.text(sprintf("%.1f", subset.matrix[i, j]), x, y, gp = gpar(fontsize = 0))},
             heatmap_legend_param = list(
               title = "Fold change (CSF/GM)"))
pdf(file = paste(resultsdirectory,"plots/Differential expression/Heatmap/XX.pdf", sep = "/"), width = 12, height = 12)
draw(hm)
dev.off()
```
