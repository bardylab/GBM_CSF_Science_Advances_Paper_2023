...
FP <- FeaturePlot(Bardy_10x, features = "NUPR1", pt.size = 0.2, label = FALSE, repel = FALSE, split.by = "Condition", cols = c("lightgrey","red"), order = TRUE, keep.scale = "all") & ##you can replace the group.by variable to any column on the metadata
  ...
  coord_cartesian(xlim = c(-15,15), ylim = c(-15, 15)) 
FP 
gsave(FP, filename = paste(resultsdirectory, "/plots/UMAP/NUPR1_splitbyCondition_NoLegend.pdf", sep = "/"), height = 6, width = 12, dpi = 300, useDingbats = FALSE)
...
NUPR1_markers <- NUPR1_markers[,-c(1:7)]

L2FC_NUPR1markers <- lapply(CSF_markers[3:12], function(x){
  x[NUPR1_markers$all_genes,]
})

##Create a matrix containing the top genes from the differential expression of all cells
Fold_change <- data.frame(matrix(ncol = 10, nrow=44))
col_names <- c("111","134", "159", "448", "468", "469","497","BAH1","MN1", "HW1")
colnames(Fold_change) <- col_names
rownames(Fold_change) <- NUPR1_markers$all_genes
...
h <- Heatmap(as.matrix(Fold_change), show_row_names = TRUE,
             ...
             rect_gp = gpar(col = "white", lwd = 1),
             cell_fun = function(j, i, x, y, width, height, fill) {
       grid.text(sprintf("%.1f", Fold_change[i, j]), x, y, gp = gpar(fontsize = 10))},
            heatmap_legend_param = list(
...
genes_list <- getBM(
  ...
  mart = mart)
markers <- na.omit(merge(NUPR1_markers, genes_list, by.x = 'all_genes', by.y = "external_gene_name", all = TRUE))

## Extract raw matrix and average expression matrix 
GBM_matrix_raw <- GBM_700cells[["RNA"]]@counts
GBM_matrix_raw <- as.matrix(GBM_matrix_raw[which(rownames(GBM_matrix_raw) %in% markers$all_genes),])
GBM_matrix_raw <- GBM_matrix_raw[rowSums(GBM_matrix_raw) > 0,]

GBM_matrix_scaled <- GBM_700cells[["RNA"]]@data
GBM_matrix_scaled <- as.matrix(GBM_matrix_scaled[which(rownames(GBM_matrix_scaled) %in% markers$all_genes),])
GBM_matrix_scaled <- GBM_matrix_scaled[rowSums(GBM_matrix_scaled) > 0,]

Idents(GBM_700cells) <- GBM_700cells$Condition_PatientID
GBM_matrix_avg <- AverageExpression(GBM_700cells, slot = "data")
GBM_matrix_avg <- as.data.frame(GBM_matrix_avg[[1]])
GBM_matrix_avg <- GBM_matrix_avg[NUPR1_markers$all_genes,]
GBM_matrix_avg <- na.omit(GBM_matrix_avg)
GBM_matrix_avg <- split(GBM_matrix_avg, c("TME","CSF"))
Heatmap <- as.matrix(t(scale(t(GBM_matrix_avg))))

split.data.frame <- function(x){
  i <- x[,grepl("TME", names(x))]
  y <- x[,grepl("CSF", names(x))]
  z <- ls(i,y)
}

split.data.frame(GBM_matrix_avg)
...
ggplot(proportions_cycling, aes(x = "", y = proportions_cycling$freq, fill = proportions_cycling$Cell_Cycle)) +
  geom_col() +
  ##geom_text(aes(label = proportions_neftel$freq), position = position_stack(vjust = 1)) +
  coord_polar(theta = "y") +
  theme_void() +
...
##color_h <- colorRamp2(c(-5,-2.5,-1,0,2.5,5), c("blue","deepskyblue","deepskyblue","white","red","firebrick"))
proportions_neftel <- as.data.frame(proportions_neftel)
proportions_neftel <- reshape2::dcast(proportions_neftel, Condition_PatientID ~ Quadrant)
row.names(proportions_neftel) <- proportions_neftel$Condition_PatientID
proportions_neftel$Condition_PatientID <- NULL
head(proportions_neftel)
proportions_neftel[is.na(proportions_neftel)] <- 0
proportions_neftel <- round(proportions_neftel, digits = 1)
names <- rownames(proportions_neftel)
Heatmap <- Heatmap[,names]
row.names(proportions_neftel) == colnames(Heatmap) ## TRUE

proportions_cycling <- as.data.frame(proportions_cycling)
proportions_cycling <- reshape2::dcast(proportions_cycling, Condition_PatientID ~ Cell_Cycle)
row.names(proportions_cycling) <- proportions_cycling$Condition_PatientID
proportions_cycling$Condition_PatientID <- NULL
head(proportions_cycling)
proportions_cycling[is.na(proportions_cycling)] <- 0
proportions_cycling <- round(proportions_cycling, digits = 1)
row.names(proportions_cycling) == colnames(Heatmap) ##TRUE

bottom_anno <- HeatmapAnnotation(quadrant_persample = anno_barplot(proportions_neftel, gp = gpar(fill = c("#FECC66","#FB0207","#804003","#0F80FF"))))

bottom <- HeatmapAnnotation(cycling_persample = anno_barplot(proportions_cycling, gp = gpar(fill = c("blue","red"))))

column_ha <- HeatmapAnnotation(Condition = anno_empty(border = T))
