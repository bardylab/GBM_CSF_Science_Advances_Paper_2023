...
lapply(file_names[1:2], load, .GlobalEnv) ##Make sure you load the FILTERED GBM_ALL data - they are both named 'Bardy_10x' in R
rm(file_names)
```
...
CSF_Markers <- FindMarkers(GBM_700cells, ident.1 = "CSF", ident.2 = "TME", min.pct = 0.3, logfc.threshold = 0, min.diff.pct = 0.05) 
...
***We will now visualise analysed data. i. FIGURE 3 and SUPP FIGURE 3.***
...
CSF_Markers$diffexpressed[CSF_Markers$avg_log2FC > 0.5 & CSF_Markers$log10p_adj > 100] <- "UP"
# if log2Foldchange < -0.6 and pvalue < 0.05, set as "DOWN"
CSF_Markers$diffexpressed[CSF_Markers$avg_log2FC < -0.5 & CSF_Markers$log10p_adj > 100] <-  "DOWN"

# add a column of NAs
CSF_Markers$delabel <- NA
CSF_Markers$delabel[CSF_Markers$top20 != "NO"] <- rownames(CSF_Markers[CSF_Markers$top20 != "NO",])
...
library(factoextra)
library(rafalib)
Dist <- dist(t(TME_Treatment)) #Calculate Distance 
Clust <- hclust(Dist)
plot(Clust)
rect.hclust(Clust, h=50)
lapply(rect.hclust(Clust, h=20), length)
##Save split cluster dendogram
##pdf(file = paste(resultsdirectory, "DendAllSamples_Split(7).pdf", sep = "/"), width = 24, height = 8, onefile = TRUE) ##Run to save as pdf
clusters_FC <- rect.hclust(Clust, h=50) ##also create clusters object containing clusters and cell barcodes
...
pdf(file = paste0(resultsdirectory, "/plots/Heatmap/Treatment_Heatmap.pdf"), width = 25, height = 10, onefile = T) ##Run to save as pdf
...
```{xxx}
##Load differential expression genes
set.seed(1)   
file_names <- list.files(paste0(resultsdirectory,"/Differential Expression"), pattern = "CSF_Markers_*", full.names = F)
CSF_markers <- lapply(list.files(paste0(resultsdirectory,"/Differential Expression"), pattern = "CSF_Markers_*", full.names = F), function(x) {read.csv(paste0(resultsdirectory,"/Differential Expression/",x), header = T)})

for(i in 1:length(CSF_markers)){
  names(CSF_markers)[[i]] <- file_names[[i]]
  rownames(CSF_markers[[i]]) <- CSF_markers[[i]]$X
 CSF_markers[[i]] <- CSF_markers[[i]][,-c(1:2,4:5,8:11)]
  CSF_markers[["CSF_Markers_469_700cells_percondition_downsample.csv"]] <- CSF_markers[["CSF_Markers_469_700cells_percondition_downsample.csv"]][-c(5756),]
}

##Load marker list
NUPR1_markers <- read.csv(file = paste0(datadirectory, "/reference_sheets/NUPR1_pathway_markers.csv"))
## get gene descriptions for NUPR1 associated genes
library(biomaRt)
mart <- useMart(biomart = "ENSEMBL_MART_ENSEMBL", dataset = "hsapiens_gene_ensembl", host = "www.ensembl.org")
genes_list <- getBM(
  filters = "external_gene_name", 
  attributes = c("external_gene_name","wikigene_description"),
  values = NUPR1_markers$all_genes,
  mart = mart)
markers <- NUPR1_markers[,8:10]
markers <- merge(markers, genes_list, by.x = 'all_genes', by.y = "external_gene_name", all = TRUE)
rownames(markers) <- markers$all_genes

L2FC_NUPR1markers <- lapply(CSF_markers[3:12], function(x){
  x[markers$all_genes,]
})

##Create a matrix containing the top genes from the differential expression of all cells
Fold_change <- data.frame(matrix(ncol = 11, nrow=52))
col_names <- c("111","134", "159", "448", "468", "469","497","BAH1","MN1", "HW1","function_1")
colnames(Fold_change) <- col_names
rownames(Fold_change) <- markers$all_genes 

for(i in 1:length(L2FC_NUPR1markers)){
  names(L2FC_NUPR1markers)[[i]] <- col_names[[i]]
  names(L2FC_NUPR1markers) %in% colnames(Fold_change) ##TRUE
  Fold_change[i] <- L2FC_NUPR1markers[[i]]$avg_log2FC
}
Fold_change <- Fold_change[rowSums(is.na(Fold_change)) != ncol(Fold_change),]
markers_sub <- rownames(Fold_change)
markers_sub <- markers[markers_sub,]
Fold_change$function_1 <- markers_sub$function_1

## Function to caluclate a distance matrix by excluding NA values
dist_no_na <- function(Fold_change) {
    edist <- dist(Fold_change[,1:10]) ## calulcate dist
    edist[which(is.na(edist))] <- max(edist, na.rm=TRUE) * 1.1 ## get the maximum value, multiply by 1.1 and add to NA values
    return(edist)
}

Dist <- dist_no_na(Fold_change[,1:10])
Clust <- hclust(Dist)
plot(Clust)

pdf(file = paste0(resultsdirectory, "/plots/Heatmap/Heatmap_NUPR1_associated_genes_DE_percellline.pdf"), width = 12, height = 12, onefile = T) ##Run to save as pdf
color_h <- colorRamp2(c(-1,0,2.5), c("blue","white","red"))
function_col = structure(brewer.pal(length(unique(markers$function_1)), "Set3"), 
    names = unique(markers$function_1))

bottom_anno <- HeatmapAnnotation(blank = anno_empty(border = FALSE))
description <- rowAnnotation(text = anno_text(Fold_change$function_1, location = 0, just = "left"),
    row_km = 3))

h <- Heatmap(as.matrix(Fold_change[,1:10]), show_row_names = TRUE,
             show_column_names = T, 
             cluster_columns = T,
             ##cluster_columns = function(m) as.dendrogram(Clust), 
             column_dend_reorder = T,
             row_dend_reorder = F,
             cluster_rows = function(m) as.dendrogram(Clust),
             show_row_dend = TRUE, 
             show_column_dend = TRUE,
             column_dend_height = unit(1, "cm"),
             row_dend_width = unit(2, "cm"),
             ...
             ##bottom_annotation = c(bottom_anno,bottom),
             na_col = "grey89",
             row_names_gp =  gpar(fontsize = 18),
             rect_gp = gpar(col = "white", lwd = 1),
             cell_fun = function(j, i, x, y, width, height, fill) {
       grid.text(sprintf("%.1f", Fold_change[,1:10][i, j]), x, y, gp = gpar(fontsize = 10))},
            heatmap_legend_param = list(
             title = "Log2FC", at = c(-1,0,1,2)))
h + Heatmap(Fold_change$function_1, name = "function", col = function_col)
dev.off()

### Generate annotations for the heatmap ####
GBM_metadata <- GBM_700cells@meta.data
GBM_metadata <- GBM_metadata[,-c(1:3,7:26,28:31,34:36,39:40,42:48,50:52,55)]
...
proportions_neftel <- GBM_metadata %>%
  ...
  summarise(n = n()) %>%
  mutate(freq = (n/sum(n))*100)
proportions_neftel$freq <- round(proportions_neftel$freq, 0)

## Create pie charts of cell state proportions
pie <- ggplot(proportions_neftel, aes(x = "", y = proportions_neftel$freq, fill = proportions_neftel$Quadrant)) +
  geom_col() +
  geom_text(aes(label = proportions_neftel$freq), position = position_stack(vjust = 0), color = "white") +
  coord_polar(theta = "y") +
  theme_void() +
  facet_grid(Condition ~ PatientID) +
  scale_fill_manual(values = c("#FECC66","#FB0207","#804003","#0F80FF")) + theme(legend.position = "none")
pie
ggsave(pie, filename = paste(resultsdirectory, "plots/Heatmap/Piecharts_CellState.pdf", sep = "/"), height = 6, width = 12, dpi = 300, useDingbats = FALSE)

proportions_cycling <- GBM_metadata %>%
  group_by(Condition, PatientID, Cell_Cycle) %>%
  summarise(n = n()) %>%
  mutate(freq = (n/sum(n))*100)
proportions_cycling$freq <- round(proportions_cycling$freq, 0)

## Create pie charts of cell_cycle  proportions
pie <- ggplot(proportions_cycling, aes(x = "", y = proportions_cycling$freq, fill = proportions_cycling$Cell_Cycle)) +
  geom_col() +
  geom_text(aes(label = proportions_cycling$freq), position = position_stack(vjust = 0), color = "white") +
  coord_polar(theta = "y") +
  theme_void() +
  facet_grid(Condition ~ PatientID) +
  scale_fill_manual(values = c("blue","red")) + theme(legend.position = "none")
ggsave(pie, filename = paste(resultsdirectory, "plots/Heatmap/Piecharts_Cycling.pdf", sep = "/"), height = 6, width = 12, dpi = 300, useDingbats = FALSE)

GBM_metadata <- GBM_metadata[!duplicated(GBM_metadata$Condition_PatientID),]
GBM_metadata <- GBM_metadata[,-c(4:13)]

## Heatmap of NUPR1 markers ####


## Extract average read counts per patient_condition
all.genes <- rownames(GBM_700cells)
GBM_700cells <- ScaleData(GBM_700cells, features = all.genes)
Idents(GBM_700cells) <- GBM_700cells$Condition_PatientID

##Idents(GBM_700cells) <- GBM_700cells$Condition
GBM_matrix_avg <- AverageExpression(GBM_700cells, slot = "counts")
GBM_matrix_avg <- as.data.frame(GBM_matrix_avg[[1]])
GBM_matrix_avg <- GBM_matrix_avg[NUPR1_markers$all_genes,]
GBM_matrix_avg <- GBM_matrix_avg[rowSums(is.na(GBM_matrix_avg)) != ncol(GBM_matrix_avg),]
GBM_matrix_avg <- as.data.frame(t(GBM_matrix_avg))
GBM_matrix_avg$sample <- rownames(GBM_matrix_avg)
GBM_matrix_avg <- GBM_matrix_avg %>% separate(sample, c("Patient","Condition"), '_')
GBM_matrix_avg <- split(GBM_matrix_avg, GBM_matrix_avg$Patient)
write.csv(as.data.frame(GBM_matrix_avg), file = paste0(resultsdirectory, "/Differential Expression/GBM_Average_Matrix.csv"), quote = FALSE)


x <- lapply(GBM_matrix_avg, function(x){
  as.data.frame(t(x))
  x[-c(51:52),]
})
...
### Run this if you want to process TME and CSF separately #####
TME_avg <- GBM_matrix_avg[,grepl("TME", colnames(GBM_matrix_avg))]
TME_avg$Average_TME <- rowMeans(TME_avg)
CSF_avg <- GBM_matrix_avg[,grepl("CSF", colnames(GBM_matrix_avg))]
CSF_avg$Average_CSF <- rowMeans(CSF_avg)

Average_matrix <- GBM_matrix_avg[rowSums(is.na(GBM_matrix_avg)) != ncol(GBM_matrix_avg),]

mean_per_gene <- function(x){
  x/TME_avg$Average_TME
}
Average_matrix <- mean_per_gene(Average_matrix)
Heatmap <- data.frame(log2(Average_matrix+1))

Average_matrix <- list(TME_avg,CSF_avg)
names(Average_matrix) <- c("TME","CSF")

##col_names <- c("BAH1","MN1", "HW1","111","134", "159", "448", "468", "469","497")
for(i in 1:length(Average_matrix)){
  Average_matrix[[i]] <- Average_matrix[[i]][,sort(colnames(Average_matrix[[i]]))]
  ##colnames(Average_matrix[[i]]) <- col_names
  Average_matrix[[i]] <- Average_matrix[[i]][rowSums(is.na(Average_matrix[[i]])) != ncol(Average_matrix[[i]]),]
  ##Average_matrix[[i]] <- as.matrix(t(scale(t(Average_matrix[[i]]))))
}

Heatmap <- cbind(Average_matrix[[1]], Average_matrix[[2]])

#### Scale matrix ####
Heatmap <- data.frame(t(scale(t(GBM_matrix_avg))))
##Heatmap <- GBM_matrix_avg
##Heatmap <- data.frame(scale(GBM_matrix_avg))
markers <- na.omit(markers[rownames(Heatmap),])
Heatmap <- Heatmap[rowSums(is.na(Heatmap)) != ncol(Heatmap),]
Heatmap$description <- markers$function_1

## Function to caluclate a distance matrix by excluding NA values
dist_no_na <- function(Heatmap) {
    edist <- dist(Heatmap[,1:20]) ## calulcate dist
    edist[which(is.na(edist))] <- max(edist, na.rm=TRUE) * 1.1 ## get the maximum value, multiply by 1.1 and add to NA values
    return(edist)
}

Dist <- dist_no_na(Heatmap[,1:20])
Clust <- hclust(Dist)
plot(Clust)
...
color_h <- colorRamp2(c(-1,0,1,2), c("blue","white","red","firebrick"))
color_h <- colorRamp2(c(-1,0,2.5), c("blue","white","red"))
##color_h <- colorRamp2(c(0,2,4,6,8), c("white","red","red","firebrick","firebrick"))
bottom_anno <- HeatmapAnnotation(blank = anno_empty(border = FALSE))
description <- rowAnnotation(text = anno_text(Heatmap$description, location = 0, just = "left"),
    row_km = 3))

pdf(file = paste0(resultsdirectory, "/plots/Heatmap/Heatmap_Scaled_Avg_Counts.pdf"), width = 12, height = 12, onefile = T) ##Run to save as pdf
h <- Heatmap(as.matrix(Heatmap[,1:20]), 
             ##as.matrix(Heatmap[,1:2]),
             show_row_names = T,
             show_column_names = T, 
             cluster_columns = T,
             ##cluster_columns = function(m) as.dendrogram(Clust), 
            ...
            row_names_side = "left",
             ...
             rect_gp = gpar(col = "white", lwd = 1),
             cell_fun = function(j, i, x, y, width, height, fill) {
       grid.text(sprintf("%.1f", Heatmap[,1:20][i, j]), x, y, gp = gpar(fontsize = 10))},
            heatmap_legend_param = list(
             title = "Scaled average counts", at = c(-1,0,1,2)))
h + Heatmap(Heatmap$description, name = "function", col = function_col)

dev.off()

## Extract raw matrix and average expression matrix 
GBM_matrix_raw <- GBM_700cells[["RNA"]]@counts
GBM_matrix_raw <- as.matrix(GBM_matrix_raw[which(rownames(GBM_matrix_raw) %in% NUPR1_markers$all_genes),])
GBM_matrix_raw <- GBM_matrix_raw[rowSums(GBM_matrix_raw) > 0,]

GBM_matrix_scaled <- GBM_700cells[["RNA"]]@data
GBM_matrix_scaled <- as.matrix(GBM_matrix_scaled[which(rownames(GBM_matrix_scaled) %in% NUPR1_markers$all_genes),])
GBM_matrix_scaled <- GBM_matrix_scaled[rowSums(GBM_matrix_scaled) > 0,]
```
