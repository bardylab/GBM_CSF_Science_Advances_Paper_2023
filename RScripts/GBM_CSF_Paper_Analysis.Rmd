...
subset.matrix <- seurat_cluster.averages[["RNA"]][c("NUPR1","RELB","DNMT1"),] ## Currently what Brett has for qPCR
...
log2_subset.matrix_FC <- log2(subset.matrix_FC)

## Visualise the expression of TFP targets
TFP_targets <- read.csv(file = paste0(datadirectory, "/reference_sheets/TFP_targets.csv"))
subset.matrix <- seurat_cluster.averages[["RNA"]][rownames(seurat_cluster.averages[["RNA"]]) %in% TFP_targets$Genes == "TRUE",]


library(stringr)
subset.matrix <- as.data.frame(t(subset.matrix))
subset.matrix[c("PatientID", "Condition")] <- str_split_fixed(rownames(subset.matrix), '_', 2)
write.csv(subset.matrix, file = paste0(resultsdirectory, "/GBM_All_sheets/TFP_targets_average_expression_GBM_700cells.csv"))
...
subset.matrix_FC <- as.matrix(t(round(subset.matrix_FC,2)))
colnames(subset.matrix_FC) <- c("BAH1","HW1","MN1","111","134","159","448","468","469","497")
subset.matrix_FC[is.nan(subset.matrix_FC)] <- 0
subset.matrix_FC[!is.finite(subset.matrix_FC)] <- 0

## Create Heatmap annotations
color_h <- colorRamp2(c(0,1,1.1,2.5,5,7.5), c("white","lightblue","gold2","orange","coral","red"))
...
hm <- ComplexHeatmap::Heatmap(subset.matrix_FC,
               ...
               title = "Fold change of average expression"))
## c(-2, 0, 2, 4)
pdf(file = paste(resultsdirectory,"plots/Differential expression/Heatmap/LogFC_NUPR1_target_genes_DNMT1_RELB.pdf", sep = "/"), width = 12, height = 12)
draw(hm)
dev.off()

## We create a separate heatmap to visualise chord plot results 
...
## Draw pie charts for the proportions of MES, AC, NPC and OPC-like cells.
...
i. ***We will now visualise analysed data. i. FIGURE 3 and SUPP FIGURE 4.***
...
###SUPP FIGURE 4A: UMAP plot to visualise Cyclcing and NonCycling Cells ####
...
##SUPP FIGURE 4B: Feature Plot by to visualise  expression of MKI67
...
####SUPP FIGURE 5I:UMAP Plot by Quadrant####
...
iii. ***FIGURE 4 and SUPP FIGURE 6.***

```{RNA seq Plots for Figure 4 and Supp Figure 6}
...
## SUPP FIGURE 6E and F: Stem cell gene expression ####
...
iv. ***FIGURE 5 and SUPP FIGURE 7.***

```{RNA seq Plots for Figure 5 and Supp Figure 7}
...
### SUPP FIGURE 7A: Volcano Plot for individual cell lines ####
...
v. ***FIGURE 6 and SUPP FIGURE AND 10.***
```{RNA seq Plots for Figure 6 and Supp Figure 9 and 10}
...
## SUPP FIGURE 9A: Violin Plot showing expression of NUPR1 across quadrants. ####
...
RP <- RidgePlot(GBM_700cells, features = "SLC22A17", group.by = "Condition", sort = "PatientID", log = F)  + ##you can replace the group.by variable to any column on the metadata. Change to "Quiescent.Score" for Figure 3I. 
  theme(legend.position = "none", legend.text = element_text(size = 16), legend.justification = "center")
RP
ggsave(RP, filename = paste0(resultsdirectory,"/plots/Scatterplots/GBM_RP_ProliferationScore_NoLegend.pdf", sep = "/"), height = 6, width = 6, dpi = 300, useDingbats = FALSE)

## SUPP FIGURE 10E: Hierarchical clustering and heatmap of treatment response in GM and CSF. ####
...
vi. ***SUPP FIGURE 15 - SANTB000134 NME expression***
```{We will now look at Average NUPR1 expression in SANTB00134 NM, TME and CSF}
